pipeline {
    ...
    environment {
        ...
        IMAGE_NAME = "${REGISTRY_HOST}/4-key-metrics-dashboard:v${env.BUILD_NUMBER}-${env.CURRENT_DATE}"
    }
        ...
        stage('Deploy to DEV') {
            steps {
                ...
                sh '''
                    ...
                    docker run --name $container_name -d -p 80:80 --net=4km-docker_default $IMAGE_NAME
                '''
            }
        }
        ...
        stage('Deploy to UAT') {
                ...
                timeout(5) {
                    input 'Continue to deploy to UAT?'
								...
								sh '''
										#!/bin/bash
										container_name=4km-dashboard-uat
										existing_container_id="$(docker ps -aqf name=$container_name)"
										if [ "$existing_container_id" ]; then
												docker container stop $existing_container_id
												docker container rm $existing_container_id
										fi
										docker run --name $container_name -d -p 9004:80 --net=4km-docker_default $IMAGE_NAME
								'''
						}
        }
    }
